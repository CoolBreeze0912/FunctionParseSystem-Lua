...
function StringUtil.split_ex(str, token)
    if (StringUtil.IsNilOrEmpty(token)) then
        return {str}
    end
    ...
    for st, sp in function()
        return string.find(str, token, pos, true)
    end do
...
--去掉字符串左右空白字符
function StringUtil.TrimWhiteSpace(str)
    if str == nil then
        return ""
    end
